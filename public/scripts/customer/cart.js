  ...
  input.addEventListener('change', function() {
    axios.put('/cart', {
      ...
      sl: parseInt(this.value)
        ...
        let priceText = document.querySelector(`.cart-price-item[data-product="${this.dataset.product}"]`);
        let donGia = document.querySelector(`.cart-dongia[data-product="${this.dataset.product}"]`);
        priceText.innerHTML = parseInt(donGia.innerHTML) * parseInt(this.value);
        totalPrice.innerHTML = getSumPrice();
        let sumPro = getSumProduct();
        totalProCheck.innerHTML = `Tổng tiền hàng (${sumPro} sản phẩm):`;
        let numAll = soluongInput.reduce((acc, cur) => acc + parseInt(cur.value), 0);

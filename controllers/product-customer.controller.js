...
module.exports.getIndex = async (req, res, next) => {
	try {
		...
		// lấy thông tin mini cart
		// let dataCart = null;
		// if (!req.user) {
		// 	dataCart = await querySQL('call SP_SELECT_CART(?)', [1]);
		// }

		res.render('customer/index', {
			title: 'ShopOP',
			productList: data[0] // sản phẩm gợi ý
			// cartNum: dataCart[0][0], // tổng số lượng sản phẩm trong cart
			// cartProduct: dataCart[1] // sản phẩm trong giỏ
		});
	} catch (err) {
		next(err);
	}
